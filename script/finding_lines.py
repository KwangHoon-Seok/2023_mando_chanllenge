...
def rad_of_curvature(left_line, right_line):
    ...
    ym_per_pix = 11 / 480  # meters per pixel in y dimension
    xm_per_pix = 3.5*(480/640) / width_lanes  # meters per pixel in x dimension
...
def lane_median(left_line, right_line):
    leftx, rightx = left_line.allx, right_line.allx
        
    median_array = rightx - 75
    median = median_array[len(rightx)//2]
    return median

def draw_follower(median, img):
    midlane = cv2.line(img, (320, 0), (320, 480), (0, 0, 255), 5)
    result = cv2.line(midlane, (int(median), 0), (int(median), 480), (255, 0, 0), 5)
    return result
...
def blind_search(b_img, left_line, right_line):
    ...
    if(len(win_left_lane)>= len(win_right_lane)):
        left_line.follower = True
        right_line.follower = False
    else:
        left_line.follower = False
        right_line.follower = True

    output[lefty, leftx] = [255, 0, 0]
    output[righty, rightx] = [0, 0, 255]

    if((leftx != 0).any() & (lefty != 0).any() & (rightx != 0).any() & (righty != 0).any()):
...
def find_LR_lines(binary_img, left_line, right_line):
    ...
    output = blind_search(binary_img, left_line, right_line)
    if output.shape[0] == 0:
        output = np.array([])
    return output
